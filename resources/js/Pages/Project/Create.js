...
const Create = () => {
  const {users, departments, clients, errors } = usePage();
  const [sending, setSending] = useState(false);

  const [values, setValues] = useState({
    name:  '',
    description: '',
    contract_sum_vat_exclusive: 0,
    contract_sum_vat_inclusive: 0,
    client_id: '',
    department_id: '',
    userdetail_id: '',
    start_date: '',
  ...
  function handleSubmit(e) {
    e.preventDefault();
    setSending(true);
    Inertia.post(route('projects.store'), values).then(() => {
      setSending(false);
    });
  }

  return (
    <BlockCard title="Create Project">
